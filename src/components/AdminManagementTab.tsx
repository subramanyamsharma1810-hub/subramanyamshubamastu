...
export default function AdminManagementTab({ onAdminCountChange, currentAdmin }: AdminManagementTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Transactional Email Diagnostic Panel (Last 10 ZeptoMail Dispatches) */}
      <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-sm space-y-4 text-left">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-[#362B5A]/10 border border-[#362B5A]/20 flex items-center justify-center text-[#362B5A]">
              <Mail className="w-5 h-5" />
            </div>
            <div>
              <h4 className="text-sm font-black text-[#362B5A]">
                Transactional Email Diagnostic Log (Last 10 ZeptoMail Attempts)
              </h4>
              <p className="text-xs text-gray-500">
                Real-time tracking of OTP verifications, password resets, and test dispatches with status, timestamps, and error diagnostics.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-xl border border-gray-200 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer self-start sm:self-auto"
          >
        ...
        {emailLogs.length === 0 ? (
          <div className="py-8 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            ...
            <p className="text-xs font-bold text-gray-600">No transactional emails recorded in current server memory session.</p>
            <p className="text-[11px] text-gray-400 mt-0.5">Send a test email above or request an OTP on the login screen to record logs.</p>
          </div>
        ) : (
          <div className="overflow-x-auto rounded-2xl border border-gray-200">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-gray-50 text-gray-600 border-b border-gray-200 font-extrabold uppercase text-[10px] tracking-wider">
                  <th className="p-3">Status</th>
                  <th className="p-3">Timestamp</th>
                  <th className="p-3">Type / Subject</th>
                  <th className="p-3">Recipient</th>
                  <th className="p-3">Provider</th>
                  <th className="p-3">Error / Response Details</th>
                </tr>
              </thead>
                  ...
                  <tr key={log.id} className="hover:bg-gray-50/80 transition-colors">
                    <td className="p-3 whitespace-nowrap">
                      {log.success ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-extrabold bg-red-100 text-red-800 border border-red-300">
                    ...
                    <td className="p-3">
                      <div className="font-bold text-[#362B5A]">{log.type}</div>
                      <div className="text-[11px] text-gray-500 truncate max-w-xs">{log.subject}</div>
                    </td>
                    ...
                    <td className="p-3 font-mono text-[11px]">
                      {log.success ? (
                        <span className="text-emerald-700 truncate max-w-xs block" title={JSON.stringify(log.details)}>
                          {JSON.stringify(log.details || "Dispatched successfully")}
                        </span>
                      ) : (
                        <span className="text-red-600 bg-red-50 p-1.5 rounded block max-w-xs truncate" title={JSON.stringify(log.error)}>
                          {JSON.stringify(log.error || "Unknown error")}
                        </span>
                      )}
                    </td>
                  </tr>
